...
import os
import random
from AI import query_ai  # Import the query_ai function from AI.py
import asyncio
...
@bot.tree.command(name="ai", description="Query the Star Citizen Helper AI")
async def ai(interaction: discord.Interaction, query: str):
    ...
    response = await query_ai(query)  # Await the asynchronous query_ai function
    await paginate_response(interaction, response)

async def paginate_response(interaction: discord.Interaction, text: str, max_length: int = 2000):
    """Helper function to paginate long responses."""
